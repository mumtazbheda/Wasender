...
export function detectColumns(headers: string[]) {
  ...
  return {
    ...
    // Feedback: match "Zoha Feedback 1", "Zoha Feedback", or bare "Zoha" (same for Ahmed/Asma)
    zoha_feedback_1: h.findIndex(x => x.includes('zoha feedback 1') || (x.includes('zoha feedback') && !x.includes('email')) || x === 'zoha'),
    zoha_feedback_2: h.findIndex(x => x.includes('zoha feedback 2')),
    zoha_feedback_3: h.findIndex(x => x.includes('zoha feedback 3')),
    ahmed_feedback_1: h.findIndex(x => x.includes('ahmed feedback 1') || x.includes('ahmed feedback') || x === 'ahmed'),
    ahmed_feedback_2: h.findIndex(x => x.includes('ahmed feedback 2')),
    ahmed_feedback_3: h.findIndex(x => x.includes('ahmed feedback 3')),
    ...
    asma_feedback_1: h.findIndex(x => x.includes('asma feedback 1') || x.includes('asma feedback') || x === 'asma'),
    asma_feedback_2: h.findIndex(x => x.includes('asma feedback 2')),
    asma_feedback_3: h.findIndex(x => x.includes('asma feedback 3')),
    // "status" bare column only — exclude all compound status column names (including furnishing)
    status: h.findIndex(x => x.includes('status') && !x.includes('listing') && !x.includes('rental') && !x.includes('occupancy') && !x.includes('vacancy') && !x.includes('vam') && !x.includes('contract') && !x.includes('transaction') && !x.includes('furnish')),
    ...
    // rent_price: exclude pf/bayut/asking prefixed price columns
    rent_price: h.findIndex(x => (x.includes('rent price') || x.includes('rent_price')) && !x.includes('pf') && !x.includes('bayut') && !x.includes('asking')),
    ...
    // asking_sale_price: use specific "asking sale" only — exclude pf/bayut portal price columns
    asking_sale_price: h.findIndex(x => (x.includes('asking sale') || x.includes('asking_sale')) && !x.includes('pf') && !x.includes('bayut')),
    ...
    // vam_listing_status: must contain both "vam" and "listing"
    ...
    // project_name_en: match "project name", "project_name_en", or exact bare "project"
    project_name_en: h.findIndex(x => x.includes('project_name_en') || x.includes('project name') || x === 'project'),
    bayut_rent: h.findIndex(x => x.includes('bayut rent') && !x.includes('price') && !x.includes('link') && !x.includes('day')),
    bayut_sale: h.findIndex(x => x.includes('bayut sale') && !x.includes('price') && !x.includes('link') && !x.includes('day')),
    pf_rent: h.findIndex(x => x.includes('pf rent') && !x.includes('price') && !x.includes('link') && !x.includes('day')),
    pf_sale: h.findIndex(x => x.includes('pf sale') && !x.includes('price') && !x.includes('link') && !x.includes('day')),
    ...
    bayut_links: h.findIndex(x => x.includes('bayut') && x.includes('link') && !x.includes('day')),
    pf_links: h.findIndex(x => x.includes('pf') && x.includes('link') && !x.includes('day')),
